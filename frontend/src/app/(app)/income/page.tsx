...
import { 
  TrendingUp, Plus, Trash2, Calendar, Wallet, 
  Briefcase, DollarSign, PiggyBank, TrendingDown,
...
export default function IncomePage() {
  ...
  const handleDelete = async (id: number) => {
    if (!confirm("Are you sure? This will deduct the amount from the wallet.")) return;
    try {
  ...
  const stats = useMemo(() => {
    ...
    const lastMonthTotal = lastMonthIncomes.reduce((sum, i) => sum + i.amount, 0);

    // Growth calculation
    const growth = lastMonthTotal > 0 ? ((thisMonthTotal - lastMonthTotal) / lastMonthTotal) * 100 : 0;

    // Average income per month
    const monthsWithIncome = new Set(incomes.map(i => {
  ...
  const getSourceIcon = (source: string) => {
    ...
    if (lowerSource.includes('salary') || lowerSource.includes('wage')) return Briefcase;
    if (lowerSource.includes('freelance') || lowerSource.includes('project')) return DollarSign;
    if (lowerSource.includes('invest') || lowerSource.includes('dividend')) return TrendingUp;
    return PiggyBank;
  };
    ...
    <div className="space-y-8">
      ...
      >
        <div>
          ...
          <p className="text-muted-foreground mt-1">Monitor your earnings and cash flow</p>
        </div>
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
        ...
        >
          <Card className="glass-card border-none bg-gradient-to-br from-green-500/20 to-emerald-500/20 overflow-hidden relative group">
            ...
            <CardContent className="p-6 relative z-10">
              ...
              <p className="text-3xl font-bold text-green-400">৳{stats.total.toLocaleString()}</p>
              ...
              <div className="flex items-center gap-1 mt-1">
                {stats.growth >= 0 ? (
                  <>
                    ...
                    <p className="text-xs text-green-400">+{stats.growth.toFixed(1)}% from last month</p>
                  </>
                ) : (
                  <>
                    <ArrowDownRight className="h-3 w-3 text-red-400" />
                    <p className="text-xs text-red-400">{stats.growth.toFixed(1)}% from last month</p>
                  </>
                )}
              </div>
            </CardContent>
        ...
        >
          <Card className="glass-card border-none overflow-hidden relative group">
            <CardContent className="p-6">
              ...
              <p className="text-3xl font-bold text-blue-400">৳{stats.avgMonthly.toFixed(0).toLocaleString()}</p>
        ...
        >
          <Card className="glass-card border-none overflow-hidden relative group">
            <CardContent className="p-6">
              ...
              <p className="text-3xl font-bold text-purple-400">৳{stats.lastMonth.toLocaleString()}</p>
                ...
                {incomes.map((income, index) => {
                  ...
                  return (
                    <motion.div
                      ...
                      className="flex items-center justify-between p-4 rounded-lg bg-gradient-to-r from-green-500/5 to-emerald-500/5 hover:from-green-500/10 hover:to-emerald-500/10 border border-green-500/10 dark:border-green-500/20 transition-all group"
                    >
                          ...
                          <div className="flex items-center gap-3 text-xs text-muted-foreground flex-wrap">
                            ...
                            {income.walletName && (
                              <span className="flex items-center gap-1 px-2 py-0.5 rounded-full bg-indigo-500/10 text-indigo-400 border border-indigo-500/20">
                                <Wallet className="h-3 w-3" /> {income.walletName}
                              </span>
                            )}
                            {income.description && (
                              <span className="truncate max-w-xs">{income.description}</span>
                            )}
                          </div>
                        </div>
                      </div>
                      <div className="flex items-center gap-4">
                        <span className="text-2xl font-bold text-green-400">+৳{income.amount.toLocaleString()}</span>
                        <Button 
                          variant="ghost" 
                          size="icon" 
