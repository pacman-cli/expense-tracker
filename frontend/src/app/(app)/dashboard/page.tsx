...
import { Activity, AlertTriangle, ArrowRight, Calendar, DollarSign, Plus, Repeat, Sparkles, TrendingUp, Wallet } from "lucide-react"
...
export default function DashboardPage() {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      if (savedGoals) {
        const goals: SavingsGoal[] = JSON.parse(savedGoals)
        if (goals.length > 0) {
          score += 10
          const hasProgress = goals.some((g: SavingsGoal) => g.currentAmount > 0)
