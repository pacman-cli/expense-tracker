...
export default function NudgesPage() {
  ...
  const fetchNudges = async () => {
    try {
      ...
      toast.error(getErrorMessage(error, "Failed to load nudges"));
    } finally {
      setLoading(false);
    }
  };
  ...
  const fetchStats = async () => {
    try {
      ...
      toast.error(getErrorMessage(error, "Failed to fetch stats"));
    }
  };
  ...
  const generateNudges = async () => {
    setGenerating(true);
    try {
      const response = await api.post("/nudges/generate");
      const count = response.data?.count || 0;
      await fetchNudges();
      await fetchStats();
      
      if (count > 0) {
        toast.success(`Successfully generated ${count} smart nudge${count > 1 ? 's' : ''}!`);
      } else {
        toast.info("No new nudges generated. Keep tracking expenses to receive personalized insights!");
      }
    } catch (error) {
  ...
  const markAsRead = async (id: number) => {
    try {
      ...
      toast.error(getErrorMessage(error, "Failed to update nudge"));
    }
  };
  ...
  const dismissNudge = async (id: number) => {
    try {
      ...
      toast.error(getErrorMessage(error, "Failed to dismiss nudge"));
    }
  };

  const getNudgeTypeConfig = (type: string) => {
    const configs: Record<string, NudgeTypeConfig> = {
      BUDGET_ALERT: {
        icon: DollarSign,
        label: "Budget Alert",
        color: "text-red-400",
  ...
  const getPriorityBadge = (priority: string) => {
    const configs: Record<string, PriorityBadgeConfig> = {
      URGENT: {
        label: "Urgent",
  ...
  const filteredNudges =
    filterType === "all" ? nudges : nudges.filter((n) => n.type === filterType);
      ...
      {stats && (
        ...
        >
          ...
          <Card className="glass-card border-none overflow-hidden group hover:scale-[1.02] transition-transform">
            <CardContent className="p-6">
              <div className="flex items-start justify-between mb-4">
                <div className="p-3 rounded-xl bg-linear-to-br from-orange-500/20 to-red-500/20">
      ...
      <div className="flex flex-wrap gap-2">
        {[
          ...
          <Button
            ...
            size="sm"
          >
            {type === "all" ? "All Nudges" : getNudgeTypeConfig(type).label}
          </Button>
        ))}
      </div>
            ...
            >
              <Card className="glass-card border-none">
                <CardContent className="p-12">
                  <div className="p-6 rounded-full bg-linear-to-br from-purple-500/20 to-pink-500/20 w-24 h-24 mx-auto mb-6 flex items-center justify-center">
                    <Zap className="h-12 w-12 text-purple-400" />
                  </div>
                  <h3 className="text-2xl font-bold mb-2">No Nudges Yet</h3>
                  <p className="text-muted-foreground mb-6">
                    Click &ldquo;Generate Nudges&rdquo; to receive personalized
                    financial insights
                  </p>
                </CardContent>
              </Card>
                ...
                >
                  <Card
                    className={`glass-card border-none overflow-hidden relative group hover:shadow-xl transition-all duration-300 ${
                      !nudge.isRead ? "border-l-4 " + typeConfig.border : ""
                    }`}
                  >
                    <div
                      className={`absolute top-0 right-0 w-32 h-32 bg-linear-to-br ${typeConfig.gradient} opacity-10 rounded-full blur-3xl group-hover:scale-150 transition-transform duration-500`}
                    />
                          ...
                          <div className="flex items-center gap-2">
                            {nudge.actionUrl && (
                              <Button
                                ...
                                size="sm"
                                className={`bg-linear-to-r ${typeConfig.gradient} hover:opacity-90 rounded-lg`}
                              >
                                Take Action
                              </Button>
                            )}
