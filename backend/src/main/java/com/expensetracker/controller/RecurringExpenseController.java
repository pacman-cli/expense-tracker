    ...
    public ResponseEntity<RecurringExpenseDTO> create(
        ...
        RecurringExpense expense = RecurringExpense.builder()
                .user(userRepository.findById(userDetails.getId())
                        .orElseThrow(
                                () -> new com.expensetracker.exception.ResourceNotFoundException("User not found")))
                .category(dto.getCategoryId() != null ? categoryRepository.findById(dto.getCategoryId()).orElse(null)
                        : null)
