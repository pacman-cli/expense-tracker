...
public class AIController {
        ...
        public ResponseEntity<?> suggestCategory(
                ...
                if (categories.isEmpty()) {
                        categories = List.of("Food", "Transport", "Entertainment", "Shopping", "Bills", "Healthcare",
                                        "Other");
                }
        ...
        public ResponseEntity<?> detectAnomalies(@AuthenticationPrincipal UserDetailsImpl userDetails) {
                ...
                List<Expense> expenses = expenseRepository.findByUserIdAndDateBetween(userId, threeMonthsAgo,
                                LocalDate.now());
                ...
                LocalDate startOfMonth = LocalDate.now().withDayOfMonth(1);
                List<Map<String, Object>> anomalies = expenseRepository
                                .findByUserIdAndDateBetween(userId, startOfMonth, LocalDate.now())
                                .stream()
                                .filter(e -> e.getCategory() != null)
                                .filter(e -> {
                                        String categoryName = e.getCategory().getName();
                                        double avg = categoryAverages.getOrDefault(categoryName, 0.0);
                                        return geminiService.isAnomalous(e.getAmount().doubleValue(), categoryName,
                                                        avg);
                                })
                                .map(e -> {
